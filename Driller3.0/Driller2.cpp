...
*/
void mergeMethod(OULinkedList<DrillingRecord>* mainLinkedList, ResizableArray<DrillingRecord>* mainRecordArray) {
...
*/
void purgeMethod(OULinkedList<DrillingRecord>* mainLinkedList, ResizableArray<DrillingRecord>* mainRecordArray) {
	...
	//loops through and adds every item in the list into the array
	while (enum2.hasNext()) {
		mainRecordArray->add(enum2.peek());
		enum2.next();
	}
	...
	*/
	while (userInput != "q" && userInput != "Q") {
		...
		if (userInput == "m" || userInput == "M") {
			mergeMethod(mainLinkedList, mainRecordArray);
		}

		//method call for if the user enters a p
		if (userInput == "p" || userInput == "P") {
			purgeMethod(mainLinkedList, mainRecordArray);
		}
